...
const MAVisualizations = {
    ...
    drawDayTradingExample(canvasId, progress = 1) {
        ...
        Canvas.drawText(ctx, '● 20 EMA', pad + 60, legendY, CONFIG.colors.gold, 10, 'left');
        Canvas.drawText(ctx, '● Entry', pad + 130, legendY, CONFIG.colors.green, 10, 'left');
        Canvas.drawText(ctx, '● Exit', pad + 185, legendY, CONFIG.colors.gold, 10, 'left');
        Canvas.drawText(ctx, '● Stop', pad + 230, legendY, CONFIG.colors.red, 10, 'left');
    },
    ...
    drawSwingTradingExample(canvasId, progress = 1) {
        ...
        if (progress > 0.5) {
            ...
            // Red dot for stop loss (below 50 SMA)
            ctx.beginPath();
            ctx.arc(entryX, stopY, 8, 0, Math.PI * 2);
            ctx.fillStyle = CONFIG.colors.red;
            ctx.fill();
        ...
        Canvas.drawText(ctx, '● 50 SMA', pad + 70, legendY, CONFIG.colors.blue, 10, 'left');
        Canvas.drawText(ctx, '● Entry', pad + 140, legendY, CONFIG.colors.green, 10, 'left');
        Canvas.drawText(ctx, '● Stop', pad + 195, legendY, CONFIG.colors.red, 10, 'left');
    },
    ...
    drawPositionTradingExample(canvasId, progress = 1) {
        ...
        if (goldenCrossIdx && progress > 0.5) {
            ...
            // Gold dot for Golden Cross (entry point)
            ctx.beginPath();
            ...
            // Red dot for stop loss (below 200 SMA)
            if (stopLossPrice) {
                const stopY = toY(stopLossPrice);
                ctx.beginPath();
                ctx.arc(crossX, stopY, 8, 0, Math.PI * 2);
                ctx.fillStyle = CONFIG.colors.red;
                ctx.fill();
                ctx.strokeStyle = '#fff';
                ctx.lineWidth = 2;
        ...
        Canvas.drawText(ctx, '● 200 SMA', pad + 65, legendY, CONFIG.colors.purple, 10, 'left');
        Canvas.drawText(ctx, '● Cross/Entry', pad + 145, legendY, CONFIG.colors.gold, 10, 'left');
        Canvas.drawText(ctx, '● Stop', pad + 230, legendY, CONFIG.colors.red, 10, 'left');
    }
};
