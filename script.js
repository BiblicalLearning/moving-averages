...
const MAVisualizations = {
    ...
    // Day Trading Example - 9 EMA / 20 EMA crossover with entry/exit
    drawDayTradingExample(canvasId, progress = 1) {
        ...
        // Draw stop loss zone FIRST (background layer) - appears early
        if (entryIdx && stopLossPrice && progress > 0.25) {
            const stopY = toY(stopLossPrice);

            // Red danger zone below stop
            ctx.fillStyle = 'rgba(231, 76, 60, 0.12)';
            ctx.fillRect(toX(entryIdx) - 10, stopY, exitIdx ? toX(exitIdx) - toX(entryIdx) + 20 : w - toX(entryIdx) - pad + 10, h - pad - stopY);

            // Stop loss line - PROMINENT
            ctx.strokeStyle = CONFIG.colors.red;
            ctx.lineWidth = 2;
            ctx.setLineDash([8, 4]);
            ctx.beginPath();
            ctx.moveTo(toX(entryIdx) - 15, stopY);
            ctx.lineTo(exitIdx ? toX(exitIdx) + 15 : w - pad, stopY);
            ctx.stroke();
            ctx.setLineDash([]);
        }

        // Draw profit zone highlight
        if (entryIdx && exitIdx && progress > 0.6) {
            ctx.fillStyle = 'rgba(39, 174, 96, 0.1)';
            ctx.fillRect(toX(entryIdx), pad, toX(exitIdx) - toX(entryIdx), toY(stopLossPrice) - pad);
        }
        ...
        // Draw STOP LOSS label - prominent and early
        if (entryIdx && stopLossPrice && progress > 0.3) {
            const stopY = toY(stopLossPrice);

            // Stop loss label box
            ctx.fillStyle = 'rgba(231, 76, 60, 0.9)';
            const labelX = toX(entryIdx) + 50;
            ctx.fillRect(labelX - 45, stopY - 10, 90, 20);

            ctx.fillStyle = '#fff';
            ctx.font = 'bold 11px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('⚠️ STOP LOSS', labelX, stopY + 4);
        }

        // Draw entry signal - appears at 35% progress
        if (entryIdx && progress > 0.35) {
            const entryX = toX(entryIdx);
            const entryY = toY(prices[entryIdx]);

            // Entry highlight circle
            ctx.beginPath();
            ...
            // Entry arrow
            Canvas.drawArrow(ctx, entryX, entryY + 40, entryX, entryY + 15, CONFIG.colors.green, 10);

            // Entry label box
            ctx.fillStyle = CONFIG.colors.green;
            ctx.fillRect(entryX - 30, entryY + 45, 60, 20);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 11px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('ENTRY', entryX, entryY + 59);
        }

        // Draw exit signal - appears at 75% progress
        if (exitIdx && progress > 0.75) {
            const exitX = toX(exitIdx);
            const exitY = toY(prices[exitIdx]);

            // Exit highlight circle
            ctx.beginPath();
            ...
            // Exit arrow
            Canvas.drawArrow(ctx, exitX, exitY - 40, exitX, exitY - 15, CONFIG.colors.gold, 10);

            // Exit label box
            ctx.fillStyle = CONFIG.colors.gold;
            ctx.fillRect(exitX - 25, exitY - 65, 50, 20);
            ctx.fillStyle = '#000';
            ctx.font = 'bold 11px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('EXIT', exitX, exitY - 51);
        }

        // Title
        Canvas.drawText(ctx, 'Day Trading: 9/20 EMA Crossover', w / 2, 20, CONFIG.colors.text, 13, 'center');

        // Legend with stop loss
        const legendY = h - 15;
        Canvas.drawText(ctx, '● 9 EMA', pad, legendY, CONFIG.colors.red, 10, 'left');
        Canvas.drawText(ctx, '● 20 EMA', pad + 70, legendY, CONFIG.colors.gold, 10, 'left');
        Canvas.drawText(ctx, '● Stop Loss', pad + 155, legendY, CONFIG.colors.red, 10, 'left');
        Canvas.drawText(ctx, '(Risk Management is Key!)', w - pad - 120, legendY, CONFIG.colors.text, 9, 'left');
    },

    // Swing Trading Example - 20 SMA / 50 SMA pullback entry with PROMINENT STOP LOSS
    drawSwingTradingExample(canvasId, progress = 1) {
        ...
        if (!this.priceCache.has('swingTradingExample2')) {
            ...
            for (let i = 0; i < 80; i++) {
                if (i < 25) {
                    ...
                    // First pullback - price drops toward 20 SMA
                    price -= 0.4 + Math.sin(i * 0.4) * 0.2;
                } else if (i < 50) {
                    // Resume uptrend after pullback
                    price += 0.5 + Math.sin(i * 0.25) * 0.3;
                } else if (i < 58) {
                    // SECOND PULLBACK - clear drop to 20 SMA area (this is our entry)
                    price -= 0.5 + Math.sin(i * 0.3) * 0.2;
                } else {
                    // Strong continuation after entry
        ...
        // Draw STOP LOSS ZONE FIRST (background) - appears at 20% progress
        if (progress > 0.20) {
            const stopY = toY(stopLossPrice);

            // Red danger zone below stop loss
            ctx.fillStyle = 'rgba(231, 76, 60, 0.15)';
            ctx.fillRect(pad, stopY, w - pad * 2, h - pad - stopY);

            // Prominent stop loss line - THICK and VISIBLE
            ctx.strokeStyle = CONFIG.colors.red;
            ctx.lineWidth = 3;
            ctx.setLineDash([10, 5]);
            ctx.beginPath();
            ctx.moveTo(pad, stopY);
            ctx.lineTo(w - pad, stopY);
            ctx.stroke();
            ctx.setLineDash([]);

            // STOP LOSS label box - CENTER of chart for visibility
            const labelX = w / 2;
            ctx.fillStyle = 'rgba(231, 76, 60, 0.95)';
            ctx.fillRect(labelX - 60, stopY - 12, 120, 24);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 12px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('⚠️ STOP LOSS', labelX, stopY + 4);

            // Additional label on right side
            ctx.fillStyle = CONFIG.colors.red;
            ctx.font = 'bold 10px Inter, sans-serif';
            ctx.textAlign = 'right';
            ctx.fillText('Below 50 SMA', w - pad - 10, stopY + 20);
        }

        // Draw support zone around 20 SMA
        if (progress > 0.4) {
            const sma20PointsZone = sma20.map((m, i) => m !== null ? { x: toX(i), y: toY(m) } : null).filter(p => p);
            if (sma20PointsZone.length > 1) {
                ctx.fillStyle = 'rgba(39, 174, 96, 0.1)';
                ctx.beginPath();
                ctx.moveTo(sma20PointsZone[0].x, sma20PointsZone[0].y - 12);
                sma20PointsZone.forEach(p => ctx.lineTo(p.x, p.y - 12));
                sma20PointsZone.slice().reverse().forEach(p => ctx.lineTo(p.x, p.y + 12));
                ctx.closePath();
                ctx.fill();
            }
        }
        ...
        // Draw entry signal at pullback - appears at 50% progress
        if (progress > 0.5) {
            const entryX = toX(entryIdx);
            ...
            // Small dot at entry point (unobtrusive)
            ctx.beginPath();
            ctx.arc(entryX, entryY, 5, 0, Math.PI * 2);
            ctx.fillStyle = CONFIG.colors.green;
            ctx.fill();

            // ENTRY label and arrow - positioned to the LEFT and ABOVE the line
            const entryLabelX = entryX - 80;
            const entryLabelY = entryY - 40;
            ctx.fillStyle = CONFIG.colors.green;
            ctx.fillRect(entryLabelX - 40, entryLabelY - 12, 75, 24);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 11px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('BUY HERE', entryLabelX - 3, entryLabelY + 3);

            // Arrow from label pointing down to entry point
            Canvas.drawArrow(ctx, entryLabelX - 3, entryLabelY + 15, entryX - 3, entryY - 8, CONFIG.colors.green, 8);
        }

        // Draw explanation text ABOVE the chart area - at 60% progress
        if (progress > 0.6) {
            const entryX = toX(entryIdx);
            const entryPrice = sma20[entryIdx] || prices[entryIdx];
            const entryY = toY(entryPrice);

            // "Pullback to 20 SMA" explanation at top
            ctx.fillStyle = CONFIG.colors.green;
            ctx.font = 'bold 11px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('↓ Price pulls back to 20 SMA = Entry', entryX + 20, pad + 25);
        }

        // Draw RISK measurement - positioned to the RIGHT side, at 65% progress
        if (progress > 0.65) {
            const entryX = toX(entryIdx);
            const entryPrice = sma20[entryIdx] || prices[entryIdx];
            const entryY = toY(entryPrice);
            ...
            // Risk line - offset to the right of entry
            const riskLineX = entryX + 80;

            // Vertical bracket showing risk distance
            ctx.strokeStyle = CONFIG.colors.red;
            ctx.lineWidth = 2;
            ctx.beginPath();
            // Top horizontal tick
            ctx.moveTo(riskLineX - 8, entryY);
            ctx.lineTo(riskLineX + 8, entryY);
            // Vertical line
            ctx.moveTo(riskLineX, entryY);
            ctx.lineTo(riskLineX, stopY);
            // Bottom horizontal tick
            ctx.moveTo(riskLineX - 8, stopY);
            ctx.lineTo(riskLineX + 8, stopY);
            ctx.stroke();

            // RISK label to the right
            ctx.fillStyle = CONFIG.colors.red;
            ctx.fillRect(riskLineX + 15, (entryY + stopY) / 2 - 10, 50, 20);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 10px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('RISK', riskLineX + 40, (entryY + stopY) / 2 + 4);
        }

        // Title
        Canvas.drawText(ctx, 'Swing Trading: Pullback to 20 SMA', w / 2, 20, CONFIG.colors.text, 13, 'center');

        // Legend with prominent stop loss mention
        const legendY = h - 15;
        Canvas.drawText(ctx, '● 20 SMA', pad, legendY, CONFIG.colors.green, 10, 'left');
        Canvas.drawText(ctx, '● 50 SMA', pad + 75, legendY, CONFIG.colors.blue, 10, 'left');
        Canvas.drawText(ctx, '⚠️ STOP below 50 SMA', pad + 155, legendY, CONFIG.colors.red, 10, 'left');
    },

    // Position Trading Example - 50/200 SMA Golden Cross with PROMINENT STOP LOSS
    drawPositionTradingExample(canvasId, progress = 1) {
        ...
        // Draw STOP LOSS ZONE FIRST - appears at 30% progress
        if (goldenCrossIdx && stopLossPrice && progress > 0.3) {
            const stopY = toY(stopLossPrice);

            // Red danger zone
            ctx.fillStyle = 'rgba(231, 76, 60, 0.12)';
            ctx.fillRect(toX(goldenCrossIdx) - 20, stopY, w - toX(goldenCrossIdx) + 20 - pad, h - pad - stopY);

            // Prominent stop loss line
            ctx.strokeStyle = CONFIG.colors.red;
            ctx.lineWidth = 3;
            ctx.setLineDash([10, 5]);
            ctx.beginPath();
            ctx.moveTo(toX(goldenCrossIdx) - 20, stopY);
            ctx.lineTo(w - pad, stopY);
            ctx.stroke();
            ctx.setLineDash([]);

            // STOP LOSS label - moved to the LEFT to give space from RISK box
            ctx.fillStyle = 'rgba(231, 76, 60, 0.95)';
            const labelX = toX(goldenCrossIdx) - 10;
            ctx.fillRect(labelX - 55, stopY - 12, 110, 24);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 11px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('⚠️ STOP LOSS', labelX, stopY + 4);
        ...
        // Draw profit zone
        if (goldenCrossIdx && stopLossPrice && progress > 0.6) {
            ctx.fillStyle = 'rgba(39, 174, 96, 0.1)';
            ctx.fillRect(toX(goldenCrossIdx), pad, w - pad - toX(goldenCrossIdx), toY(stopLossPrice) - pad);
        }
        ...
        // Draw Golden Cross signal - appears at 45% progress
        if (goldenCrossIdx && progress > 0.45) {
            const crossX = toX(goldenCrossIdx);
            const crossY = toY(sma50[goldenCrossIdx]);

            // Highlight circle at crossover
            ctx.beginPath();
            ...
            // Golden Cross label
            ctx.fillStyle = CONFIG.colors.gold;
            ctx.fillRect(crossX - 55, crossY - 42, 110, 22);
            ctx.fillStyle = '#000';
            ctx.font = 'bold 11px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('⭐ GOLDEN CROSS', crossX, crossY - 27);
        }

        // Draw entry signal - appears at 55% progress
        if (goldenCrossIdx && progress > 0.55) {
            const crossX = toX(goldenCrossIdx);
            const crossY = toY(sma50[goldenCrossIdx]);

            // Small dot at entry point (at the golden cross)
            ctx.beginPath();
            ctx.arc(crossX, crossY, 5, 0, Math.PI * 2);
            ctx.fillStyle = CONFIG.colors.green;
            ctx.fill();

            // Entry label box to the LEFT of golden cross, not overlapping
            const entryLabelX = crossX - 130;
            const entryLabelY = crossY - 50;
            ctx.fillStyle = CONFIG.colors.green;
            ctx.fillRect(entryLabelX - 30, entryLabelY - 12, 60, 24);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 11px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('ENTRY', entryLabelX, entryLabelY + 3);

            // Arrow pointing RIGHT toward the golden cross
            Canvas.drawArrow(ctx, entryLabelX + 35, entryLabelY, crossX - 20, crossY, CONFIG.colors.green, 8);

            // Risk bracket to the RIGHT - at 65% progress
            if (stopLossPrice && progress > 0.65) {
                const stopY = toY(stopLossPrice);
                const riskLineX = crossX + 120;

                // Vertical bracket showing risk distance
                ctx.strokeStyle = CONFIG.colors.red;
                ctx.lineWidth = 2;
                ctx.beginPath();
                // Top horizontal tick at entry level
                ctx.moveTo(riskLineX - 8, crossY);
                ctx.lineTo(riskLineX + 8, crossY);
                // Vertical line
                ctx.moveTo(riskLineX, crossY);
                ctx.lineTo(riskLineX, stopY);
                // Bottom horizontal tick at stop level
                ctx.moveTo(riskLineX - 8, stopY);
                ctx.lineTo(riskLineX + 8, stopY);
                ctx.stroke();

                // RISK label to the right of bracket
                ctx.fillStyle = CONFIG.colors.red;
                ctx.fillRect(riskLineX + 15, (crossY + stopY) / 2 - 10, 50, 20);
                ctx.fillStyle = '#fff';
                ctx.font = 'bold 10px Inter, sans-serif';
                ctx.textAlign = 'center';
                ctx.fillText('RISK', riskLineX + 40, (crossY + stopY) / 2 + 4);
            }
        }

        // Draw "Hold Position" text - appears at 75% progress
        if (goldenCrossIdx && progress > 0.75) {
            const holdX = toX(goldenCrossIdx) + (w - pad - toX(goldenCrossIdx)) / 2;

            ctx.fillStyle = CONFIG.colors.green + 'DD';
            ctx.font = 'bold 12px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('HOLD POSITION', holdX, pad + 30);
        }

        // Title
        Canvas.drawText(ctx, 'Position Trading: Golden Cross Entry', w / 2, 20, CONFIG.colors.text, 13, 'center');

        // Legend
        const legendY = h - 15;
        Canvas.drawText(ctx, '● 50 SMA', pad, legendY, CONFIG.colors.green, 10, 'left');
        Canvas.drawText(ctx, '● 200 SMA', pad + 75, legendY, CONFIG.colors.purple, 10, 'left');
        Canvas.drawText(ctx, '● Golden Cross', pad + 165, legendY, CONFIG.colors.gold, 10, 'left');
        Canvas.drawText(ctx, '● STOP below 200', pad + 275, legendY, CONFIG.colors.red, 10, 'left');
    }
};
